...
const Login=({islogin, login, onCheckUser=f=>f, lang})=>{
    ...
    return (
      <div className="min-vh-100 d-flex justify-content-md-center align-items-start bg-light">
      
      {(islogin && login==='admin')?
        <Redirect to="/admin" />:(islogin)?<Redirect to="/accaunt" />:
        <div className="p-2 w-100 w-sm-100 w-md-50 d-flex flex-column justify-content-center">
